            ...
            worker.addEventListener('message', function(e) {
              if (e.data.progress !== undefined) {
                ...
                if (object && object.userData.linePoints.length > 1) {
                  ...
                  // Handle unit scaling (mm/inch)
                  if (object.userData.inch) {
                    object.scale.set(25.4, 25.4, 25.4); // Convert inches to mm
                  }
      
                  // Update grid based on units
                  if (localStorage.getItem('unitsMode')) {
                    const unitsMode = localStorage.getItem('unitsMode');
                    const bbox = object.userData.bbbox2;
                    const isInch = object.userData.inch;
      
                    let minX, maxX, minY, maxY;
                    if (unitsMode === "in") {
                      minX = isInch ? bbox.min.x : bbox.min.x / 25.4;
                      maxX = isInch ? bbox.max.x : bbox.max.x / 25.4;
                      minY = isInch ? bbox.min.y : bbox.min.y / 25.4;
                      maxY = isInch ? bbox.max.y : bbox.max.y / 25.4;
                      redrawGrid(minX, maxX, minY, maxY, true);
                    } else {
                      minX = isInch ? bbox.min.x * 25.4 : bbox.min.x;
                      maxX = isInch ? bbox.max.x * 25.4 : bbox.max.x;
                      minY = isInch ? bbox.min.y * 25.4 : bbox.min.y;
                      maxY = isInch ? bbox.max.y * 25.4 : bbox.max.y;
                      redrawGrid(minX, maxX, minY, maxY, false);
                    }
                  }
      
                  // Reset view and display time estimate
                  setTimeout(function() {
                    if (webgl) $('#gcodeviewertab').click();
                    clearSceneFlag = true;
                    resetView();
      
                    // Extract and display estimated time
                    const timeremain = object.userData.totalTime;
                    if (!isNaN(timeremain)) {
                      const formattedTime = timeConvert(timeremain);
                      $('#timeRemaining').html(`${formattedTime} / ${formattedTime}`);
                      printLog(`
                        <span class='fg-red'>[GCODE Parser]</span>
                        <span class='fg-darkGreen'>
                          GCODE Preview Rendered Successfully: 
                          Total lines: <b>${object.userData.linePoints.length}</b> / 
                          Estimated Run Time: <b>${formattedTime}</b>
                        </span>
                      `);
                    }
                  }, 200);
                  ...
                  // Handle empty/invalid G-code
                  $('#3dviewicon').removeClass('fa-pulse');
                  $('#3dviewlabel').html(' 3D View');
                  printLog("<span class='fg-red'>[Error]</span> No valid G-code lines detected.");
                }
              }
            }, false);
      
            // Start the worker with G-code data
...
parseGcodeInWebWorker('car_but bigger scale.gcode');